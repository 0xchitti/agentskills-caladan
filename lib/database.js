...
export class Database {
    // Force clean state - no cache initialization with old data
    static initializeCache() {
        // Clean slate - start with empty arrays
        if (AGENTS_CACHE.length === 0 && PERSISTENT_AGENTS.length > 0) {
            // Don't load old persistent data
            console.log('Clean state maintained - no demo data loaded');
        }
        if (SKILLS_CACHE.length === 0 && PERSISTENT_SKILLS.length > 0) {
            // Don't load old persistent data
            console.log('Clean state maintained - no demo skills loaded');
        }
    }
